...
- `.claude/agents/`: 에이전트 팀 8명, `.claude/commands/`: `/blog-plan`, `/blog-write`, `/blog-index`, `/blog-report`, `/blog-refresh`

## 팀 운영 원칙 (편집장 = 메인 세션)
1. 기획 → 팩트시트 → 원고 → 검수 → 변호사 승인 → 비공개 발행 → 공개(변호사) → 색인 등록(publisher) → 측정 → 사후관리.

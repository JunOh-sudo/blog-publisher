...
1. `data/published_posts.csv` + 최신 리포트를 보고 트리거별 대상 글을 뽑는다.
   - 7일 미색인 (`published_posts.csv`의 indexed_* = N): 원인 점검(다른 글과의 중복·유사도, 얇은 본문, 금지어) → 수정 후 재요청 목록 작성. 네이버 블로그 누락은 제목·도입부 수정으로 대응(삭제 후 재발행은 변호사 승인 시에만)
   - **내용을 수정·갱신한 글은 모두 색인 재요청 목록에 넣는다** (티스토리: Search Console·서치어드바이저·다음)
   - 30일 저CTR / 90일 11~30위 / 3개월 하락 / 동일 키워드 중복
